...
The "out-of-the-box" introduction screen guides you through the basics of Gerrit
and allows installation of additional plugins downloaded from [Gerrit CI](https://gerrit-ci.gerritforge.com).
...
Note that the path `/var/gerrit/etc` may also be externally-mounted.  If this is done,
then it is expected that the "out-of-the-box" configuration will be disabled. This
happens because the "out-of-the-box" config file `/var/gerrit/etc/gerrit.config`
initialized by the Gerrit deb/rpm package is no longer available.
